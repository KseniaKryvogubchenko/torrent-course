...
module.exports = (torrent, path, port) => {
    tracker.getPeers(torrent, port, (peers) => {
        const pieces = new Pieces(torrent)
        const file = fs.openSync(path, 'w')

        // peers.forEach((peer) =>
        //     peer.port === PORT ? console.log('own') : console.log('other')
        // )
        peers.forEach((peer) => {
            console.log(peer)
            if (peer.port !== port) download(peer, torrent, pieces, file)
        })
    })
}
